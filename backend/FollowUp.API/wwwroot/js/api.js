// FollowUp Web Dashboard - API Integration
// This file handles all communication with the backend API

const API_CONFIG = {
    BASE_URL: '/api',  // Relative URL since web is served from same backend
    TOKEN_KEY: 'followup_token',
    REFRESH_TOKEN_KEY: 'followup_refresh_token',
    USER_KEY: 'followup_user'
};
...
const DEFAULT_MAP_CENTER = { latitude: 31.9, longitude: 35.2, zoom: 14 };

const municipalityApi = {
    // Cache for municipality settings
    _settings: null,

    async getDefault() {
        if (this._settings) return this._settings;
        try {
            const response = await api.get('/municipality/default');
            if (response.success && response.data) {
                this._settings = response.data;
                return this._settings;
            }
        } catch (e) {
            console.log('Failed to load municipality settings, using defaults');
        }
    ...
    async getCurrent() {
        try {
            const response = await api.get('/municipality/current');
            if (response.success && response.data) {
                return response.data;
            }
        } catch (e) {
            console.log('Failed to load municipality settings');
        }
    ...
    async getMapCenter() {
        const settings = api.isAuthenticated()
            ? await this.getCurrent()
            : await this.getDefault();
        return {
            lat: settings.centerLatitude ?? DEFAULT_MAP_CENTER.latitude,
            lng: settings.centerLongitude ?? DEFAULT_MAP_CENTER.longitude,
            zoom: settings.defaultZoom ?? DEFAULT_MAP_CENTER.zoom
        };
    }
};
